<template>
    <div class="flex flex-col lg:flex-row">
        <div class="w-full lg:w-1/2  p-4 lg:p-10  ">
            ...
            <img :src="res.image" alt="product-image" class="hover:scale-110 transition duration-300 p-4 max-h-[300px] lg:max-h-[500px] mx-auto ">
        </div>
        <div class="w-full lg:w-1/2 p-4 lg:p-10">
            <p class="font-bold text-lg lg:text-2xl text-green-600">{{ res.title }}</p>
            <!-- Rating Section -->
            <div class="flex items-center mt-4">
                ...
                <div class="flex">
                    ...
                    <span v-for="n in 5" :key="n" class="text-yellow-500">
                        <svg v-if="n <= Math.floor(res?.rating?.rate)" xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="w-5 h-5" viewBox="0 0 24 24">
                ...
                <p class="ml-2 text-gray-600 text-sm underline">{{ res?.rating?.count }} reviews</p>
            </div>
            </div>
          <!-- Add to favourites and add to cart sections -->
          <div class="flex items-center  justify-start mt-4">
                <button class=" favourite-btn flex items-center mr-1 lg:space-x-1 lg:mr-10">
                    <Icon name="carbon:favorite" class="w-5 h-5" />
                    <span>Add to favourites</span>
                </button>
                <NuxtLink to="/cart" class="cart-btn flex items-center space-x-1">
